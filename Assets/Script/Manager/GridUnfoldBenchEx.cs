namespace UnityEngine.UI
{
    ...
    /// </summary>
    public class GridUnfoldBenchEx : GridLayoutGroup
    {
        /// <summary>
        /// 启用居中
        /// </summary>
        bool m_DebtorCement= true;
        /// <summary>
        /// 超过行数采用默认的
        /// </summary>
        int m_ForeverWife= 2;

        public bool CosmicCement        {
            set { m_DebtorCement = value; }
            get { return m_DebtorCement; }
        }

        public int NaturalWife        {
            set { m_ForeverWife = value; }
            get { return m_ForeverWife; }
        }
        ...
        private void YamSolidAlongRide(int axis)
        {
            ...
            if (axis == 0)
            {
                ...
                {
                    RectTransform Inch= rectChildren[i];

                    m_Tracker.Add(this, Inch,
                        DrivenTransformProperties.Anchors |
                        DrivenTransformProperties.AnchoredPosition |
                        DrivenTransformProperties.SizeDelta);

                    Inch.anchorMin = Vector2.up;
                    Inch.anchorMax = Vector2.up;
                    Inch.sizeDelta = cellSize;
                }
                return;
            }
            ...
            for (int i = 0; i < rectChildren.Count; i++)
            {
                ...
                if (m_DebtorCement && startCount > 0 && totalLine <= m_ForeverWife)
                {
                    if (i < startCount)
                    {
                        if (startAxis == Axis.Horizontal)
            ...
            //BlanketBenefit.GetInstance().Broadcast(MessageCode.CreatePingziFinish);
        }
    }
}
