...
public class GroundingBench : MonoBehaviour
{   
    /// <summary>
    /// 瓶子完成动画
    /// </summary>
    ...
    public static void ElusiveBranchBus(GameObject Bottle,GameObject Plug,ParticleSystem SceenEffect,ParticleSystem RibbonEffect,System.Action finish)
    {
    ...
    public static IEnumerator ValidBranchBus(SkeletonGraphic Cat,SkeletonGraphic Welldone,ParticleSystem SceenEffect,ParticleSystem RibbonEffect,List<GameObject> ShowList,System.Action finish)
    {
        RibbonEffect.gameObject.SetActive(false);
        Welldone.AnimationState.ClearTracks();
        // 设置屏幕特效和彩带特效
        BoneBenefit.RimIndicate().HabitatOrgan(RealmCare.UIMusic.win_2);
        SceenEffect.gameObject.SetActive(true);
        SceenEffect.Play();
    ...
    /// <param name="AC"></param>
    public static void JoinerMeBus(GameObject Bottle,Vector3 StartPos,System.Action AC)
    {
    ...
    /// <param name="StartPos"></param>
    /// <param name="AC"></param>
    public static void JoinerLeapBus(bool Playing,GameObject Bottle,Vector3 StartPos,System.Action AC)
    {
    ...
    /// <param name="A"></param>
    /// <param name="Finish"></param>
    public static void ObviouslyPourLowerBus(GameObject Item,float A ,Vector3 EndPos,Image[] color, GameObject Plug,System.Action Finish) 
    {
        //瓶子
        A = A > 0 ? 40 : -40;
        EndPos.y += 0.5f;
    ...
    /// <param name="Finish"></param>
    public static void SearLowerBusInSpecimen(GameObject Item,float A, Image[] color, GameObject Plug, float FristCurVolme, System.Action Finish)
    {
        //瓶子
    ...
    /// <param name="Plug"></param>
    /// <param name="Finish"></param>
    public static void SearLowerBusOver(GameObject Item, Vector3 EndPos,int CurVolume, int ReadyNumber, float A, Image[] color, Image[] Gray, GameObject Plug, System.Action Finish) 
    {
        float BackTime = 0.3f;
        //瓶子
        float AToIndax = (Mathf.Abs(A) - 60) / 10;
    ...
    public static void TheAwakeQuotationSearLower(int NotVolume, Image[] color, GameObject SelectObj, colorconfigDB WaterColor, GameObject Plug, int CurVolume, float A, int FristCurVolme, int ReadyNumber, Action<int> ChangeWaterColor)
    {
        ...
        float Time = 0.35f + (AToIndax * 0.05f);
        float watertime = (4 - FristCurVolme) * 0.1f;
        ClauseLast WaterLineEffect = SelectObj.GetComponent<ClauseLast>();
        YamOre(NotVolume, WaterLineEffect.LowerWife, WaterLineEffect.LowerHandle.gameObject, () =>
        {
            Plug.SetActive(true);
    ...
    public static void YamOre(int NotVolume, GameObject WaterLine, GameObject WaterLineEffect, Action waterup, Color WaterColor, int ReadyNumber, float movetime, Action<int> ChangeWaterColor)
    {
        ...
        mainModule.startLifetime = 0.2f * ReadyNumber;
    ...
    /// <param name="Finish"></param>
    public static void GuardSquirrelBelow(GameObject Item,ParticleSystem Fx_Star,System.Action Finish)
    {
    ...
    public static void DuckBus(GameObject HandItem,Vector3 EndPos,bool UpOrDown) 
    {
    ...
    public static void HabitBus(GameObject Left,GameObject Right,System.Action finish, System.Action Otherfinish)
    {
    ...
    public static void ToErieBandBus(GameObject Fx_Shoplock, GameObject shopitem,System.Action Unlock=null)
    {
    ...
    public static void HectareBus(List<GameObject> loadingItems) 
    {
        float Lush= 0.3f;
        for (int i = 0; i < loadingItems.Count; i++)
        {
            ...
            item.transform.DOScale(1, 0.2f).SetDelay(Lush).SetEase(Ease.OutBack);
            Lush += 0.05f;
        }
    } 
}