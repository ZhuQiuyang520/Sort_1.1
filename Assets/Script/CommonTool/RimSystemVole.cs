...
{
    public static void Beast<TKey, TValue>(this IDictionary<TKey, TValue> first, IDictionary<TKey, TValue> second)
    {
    ...
    public class RimSystemVole : DeedSingleton<RimSystemVole>
{
    // Start is called before the first frame update
    public bool PantCodify;
    public bool ByRole;

    static public RimSystemVole instance;
    ...
    /// <returns></returns>
    public Vector3 WedNurseHeavy(GameObject gameobj)
    {
    ...
    /// <returns></returns>
    public GameObject RimPublishBoneCompel(Transform SelfTransform,List<Transform> listTemp)
    {
    ...
    /// <param name="p2"></param>
    /// <returns></returns>
    public float HeavyOrBadge(Vector2 p1, Vector2 p2)
    {
        Vector2 p;
        p.x = p2.x - p1.x;
        p.y = p2.y - p1.y;
    ...
    /// <returns></returns>
    public float Badge180Or360(float angle)
    {
    ...
    /// <param name="p2"></param>
    /// <returns></returns>
    public int RimFlankSiliceous(Vector2 p1, Vector2 p2)
    {
        float angle = Badge180Or360(HeavyOrBadge(p1, p2));
    ...
    /// <returns></returns>
    public Vector2 WedBrieflyJokeLook(Renderer Renderer)
    {
    ...
    public Vector2 WedBrieflyJokeLook(Image image)
    {
    ...
    /// <param name="obj"></param>
    /// <returns></returns>
    public Vector2 WedCobaltLook(GameObject obj)
    {
    ...
    /// </summary>
    /// <returns></returns>
    public Vector2 WedGroanAppetite()
    {
        
        Vector3 v = Input.mousePosition;
        float xa = v.x / Screen.width;
        float ya = v.y / Screen.height;
        float screenW = WedRotaryUtter();
        float screenH = WedRotaryUseful();
        float x = -screenW/2 + xa * screenW;
        float y = -screenH/2 + ya * screenH;
        return new Vector2(x, y);   
    }

    public int WedSpiralFresh(List<string> weightList)
    {
        float sumWeight = 0;
    ...
    public int[] RimTariffMedicine(int total, int count)
    {
    ...
    public Dictionary<string, object> SyntaxOrIndividual(string value)
    {
        if (value.Length < 1)
        {
            return null;
    ...
    public string IndividualOrSyntax(Dictionary<string, object> Info)
    {
    ...
    public int OilCureMite(string dateStart, DateTime dateEnd)
    {
    ...
    public int RowCureMite(string dateStart, DateTime dateEnd)
    {
    ...
    public TimeSpan CureMite(DateTime dateStart, DateTime dateEnd)
    {
    ...
    public Vector3 RimLegAtCart(Canvas canvas, Text text, string strFragment)
    {
        int strFragmentIndex = text.text.IndexOf(strFragment);//-1表示不包含strFragment
        Vector3 stringPos = Vector3.zero;
        if (strFragmentIndex > -1)
        {
            Vector3 firstPos = RimLegAtCart(canvas, text, strFragmentIndex);
            Vector3 lastPos = RimLegAtCart(canvas, text, strFragmentIndex + strFragment.Length);
            stringPos = (firstPos + lastPos) * 0.5f;
        }
        else
        {
            stringPos = RimLegAtCart(canvas, text, strFragmentIndex);
        }
        return stringPos;
    }

    public Vector3 RimLegAtCart(Canvas canvas, Text text, int charIndex)
    {
    ...
    /// <returns></returns>
    public float EnormousWeGrow(Vector3 pB, Vector3 pA1, Vector3 pA2)
    {
    ...
    private static byte[] Boil= { 0x12, 0x34, 0x56, 0x78, 0x90, 0xAB, 0xCD, 0xEF };
    /// <summary>
    /// DES加密字符串
    /// </summary>
    ...
    public static string ProcessDES(string encryptString, string key = "abcdefgh")
    {
        try
        {
    ...
    public static  string SomedayDES(string decryptString, string key = "13717421")
    {
        byte[] rgbKey = Encoding.UTF8.GetBytes(Application.identifier.Substring(0, 8));
        byte[] rgbIV = Boil;
