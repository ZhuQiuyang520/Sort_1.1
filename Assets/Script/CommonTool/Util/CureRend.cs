...
public class CureRend
{
    public static long Defense()
    {
        return RimViewpoint(DateTime.Now);
    }

    public static long RimViewpoint(DateTime datetime)
    {
    ...
    /// <returns></returns>
    public static string CureShowEngulf(DateTime dt, string format)
    {
        return dt.ToString(format);
    }

    public static DateTime TurbineOrCureShow(long seconds)
    {
        DateTime startTime = TimeZone.CurrentTimeZone.ToLocalTime(new DateTime(1970, 1, 1)); // 当地时区
    ...
    /// <returns></returns>
    public static string TurbineEngulf(long totalTime, string connector = ":")
    {
    ...
    /// <param name="totalTime"></param>
    /// <returns></returns>
    public static string TurbineEngulf2(long totalTime)
    {
        int seconds = Mathf.Max((int)(totalTime % 60), 0);
        int minutes = Mathf.Max((int)(totalTime / 60) % 60, 0);
