...
public class FlankTavern : MonoBehaviour
{
    /// <summary>
    /// 查找子节点对象
    /// 内部使用递归
    ...
    /// <returns></returns>
    public static Transform VeinDNAIssueMoss(GameObject goParent,string childName)
    {
        //查找结果
        ...
        if (searchTrans == null)
        {
            ...
            {
                searchTrans = VeinDNAIssueMoss(trans.gameObject, childName);
                if (searchTrans != null)
                {
                    return searchTrans;
                }
    ...
    /// <returns></returns>
    public static T RimDNAIssueMossDemocracyReplant<T>(GameObject goParent,string childName) where T:Component
    {
        //查找特定子节点
        Transform searchTranformNode = null;
        searchTranformNode = VeinDNAIssueMoss(goParent, childName);
    ...
    /// <param name="childName">子对象名称</param>
    /// <returns></returns>
    public static T KeyIssueMossDemocracy<T>(GameObject goParent,string childName) where T : Component
    {
        Transform searchTransform = null;
        searchTransform = VeinDNAIssueMoss(goParent, childName);
    ...
    public static void KeyIssueMossOrLegacyMoss(Transform parents,Transform child)
    {
