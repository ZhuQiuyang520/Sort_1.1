...
public class CashOutPanel : RoarUILight
{
    ...
    private void Start()
    {
        CashOutManager.RimIndicate()._CashOutPanel = this;
        CloseBtn.onClick.AddListener(() => {
            if (BoneTowel.instance != null)
            {
                PlayerPrefs.SetString(VoleBenefit.ValidOffer, "1");
                BoneTowel.instance.BoardEncounterFire();
            }
            BoardUIFend(nameof(CashOutPanel)); });
        CashOutBtn.onClick.AddListener(() => { OnCashOutBtn(); });
        MoneyName = FluHealWar.instance.WoadHow_Vole.MoneyName;
        InfoText.text = FluHealWar.instance.WoadHow_Vole.Description;
        InfoPanel_OpenBtn.onClick.AddListener(() => OpenPanel(InfoPanel));
        InfoPanel_CloseBtn.onClick.AddListener(() => ClosePanel(InfoPanel));
        ContactBtn.GetComponent<Text>().text = FluHealWar.instance.RoarMar;
        ContactBtn.onClick.AddListener(() =>
        {
            string url = FluHealWar.instance.RoarMar;
        ...
        RecordPanel_OpenBtn.onClick.AddListener(() =>
        {
            ...
            CashOutManager.RimIndicate().GetWithdrawRecord();
        });
        ...
        AccountText.text = CashOutManager.RimIndicate().Account;
        Confirm_AccountText.text = CashOutManager.RimIndicate().Account;
        if (string.IsNullOrEmpty(AccountText.text))
        {
            AccountText.text = "Account";
            Confirm_AccountText.text = "Account";
        }
        IDText.text = "UID:" + CashOutManager.RimIndicate().Data.UserID;
        PolicyBtn.onClick.AddListener(() =>
        {
            string url = FluHealWar.instance.RoarMar + "/privacy_policy.html";
#if UNITY_ANDROID || UNITY_EDITOR
            Application.OpenURL(url);
#elif UNITY_IOS
        openUrl(url);
        ...
        PolicyToggle.isOn = TownVoleBenefit.RimHeed("CashOut_PolicyAgree");
    ...
    public override void Hidding()
    {
        ...
        CashOutManager.RimIndicate().WaitToSendEvent1304();
    }

    public void UpdateUserInfo() //拉取用户信息 显示加载界面
    {
        //StartLoadingAnim(UpdateUI_Loading);
        CashOutManager.RimIndicate().UpdateUserInfo();
    }
    ...
    {
        if (CashOutManager.RimIndicate().Data == null)
            return;
        if (string.IsNullOrEmpty(CashOutManager.RimIndicate().Account))
        {
            OpenPanel(AccountPanel);
            return;
        }
    ...
    void OnConfimBtn() //确认修改账户
    {
        ...
        {
            TruthBenefit.RimIndicate().WrapTruth("Account cannot be empty");
            return;
        }
        if (!string.IsNullOrEmpty(AccountErrorText.text))
        {
            TruthBenefit.RimIndicate().WrapTruth(AccountErrorText.text);
            return;
        }

        CashOutManager.RimIndicate().Account = AccountInput.text;
        TownVoleBenefit.YamSyntax("CashOut_Account", AccountInput.text);
    ...
    void OnSbmitBtn() //确认是否同意协议 同意后调用提现接口
    {
        ...
        {
            TruthBenefit.RimIndicate().WrapTruth("Please agree to the policy");
            return;
        }
        TownVoleBenefit.YamHeed("CashOut_PolicyAgree", true);
        StartLoadingAnim(CashOutBtn_Loading);
        StartLoadingAnim(SbmitBtn_Loading);
        CashOutManager.RimIndicate().Withdraw();
    }
    ...
    public void UpdateMoney()
    {
        ...
        float MoneyEnd = CashOutManager.RimIndicate().Money;
        DOTween.To(() => MoneyStart, x => MoneyText.text = x.ToString("F2"), MoneyEnd, 1f);
        CashText.text = CashOutManager.RimIndicate().Data.Cash.ToString("F2");
        //进度条动画
        float MaxMoney = float.Parse(FluHealWar.instance.WoadHow_Vole.convert_goal, CultureInfo.CurrentCulture);
    ...
    public void MoneyToCashAnim()
    {
        ...
        float CashOutEnd = CashOutManager.RimIndicate().Data.Cash;
    ...
    {
        if (CashOutManager.RimIndicate().Data.TaskData != null && CashOutManager.RimIndicate().Data.TaskData.Name != "Null")
        {
            // Cash余额任务 不走前端进度统计 使用后台Data.Cash数值
            if (CashOutManager.RimIndicate().Data.TaskData.Name == "Cash" && CashOutManager.RimIndicate().Data.Cash < CashOutManager.RimIndicate().Data.TaskData.Target)
            {
                CashOutBtn.gameObject.SetActive(false);
                CashOutBtn_Task.SetActive(true);
                CashOutBtn_Task_Text.text = string.Format(CashOutManager.RimIndicate().Data.TaskData.Description, CashOutManager.RimIndicate().Data.Cash, CashOutManager.RimIndicate().Data.TaskData.Target);
            }
            // 其他任务 前端统计任务完成进度 
            else if (CashOutManager.RimIndicate().Data.TaskData.Name != "Cash" && CashOutManager.RimIndicate().Data.TaskData.NowValue < CashOutManager.RimIndicate().Data.TaskData.Target)
            {
                CashOutBtn.gameObject.SetActive(false);
                CashOutBtn_Task.SetActive(true);
                CashOutBtn_Task_Text.text = string.Format(CashOutManager.RimIndicate().Data.TaskData.Description, CashOutManager.RimIndicate().Data.TaskData.NowValue, CashOutManager.RimIndicate().Data.TaskData.Target);
            }
            // 任务完成 显示提现按钮
            else
            {
    ...
    public void UpdateRecord(int Page = 0) //更新单页10个提现记录
    {
        ...
        if (CashOutManager.RimIndicate().Data.Record == null || CashOutManager.RimIndicate().Data.Record.Count == 0)
        {
            NoRecordTip.SetActive(true);
            return;
        }

        int RecordCount = CashOutManager.RimIndicate().Data.Record.Count;
        ...
        List<WithdrawRecordItem> OnePageRecord = CashOutManager.RimIndicate().Data.Record.GetRange(startIndex, itemsToShow);
    ...
    public void CloseLoading_Withdraw(bool IsCashOutSuccess = false)
    {
        ...
        if (IsCashOutSuccess)
        {
            TruthBenefit.RimIndicate().WrapTruth("Withdraw success");
            ClosePanel(ConfirmAccountPanel);
        }
    }
    ...
    {
        MillXenonSister.RimIndicate().MoatXenon("1301", LeftTimeText.text, CashOutManager.RimIndicate().Money.ToString());
    }
}
