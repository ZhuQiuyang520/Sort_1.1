...
public class CashOutManager : DeedSingleton<CashOutManager>
{
    [Header("登录平台")]
    ...
    private void Start()
    {
        Account = TownVoleBenefit.RimSyntax("CashOut_Account");
        Money = TownVoleBenefit.RimHinge("CashOut_Money");
    }
    ...
    private void OnApplicationPause(bool pauseStatus)
    {
        ...
        if (pauseStatus && Seconds > 0)
        {
            ...
            string info = $"All {FluHealWar.instance.WoadHow_Vole.MoneyName} have been converted,Please check your rewards!";
            TribespeopleBenefit.Indicate.GuardTribespeople();
            TribespeopleBenefit.Indicate.OffshoreTribespeople(title, info, (int)Seconds);
            for (int i = 0; i < 10; i++) // 10次延时 10800秒 3小时
                TribespeopleBenefit.Indicate.OffshoreTribespeople(title, info, (int)Seconds + (i * 10800));
        }
    ...
    void TimeCount() //计时
    {
        if (Data != null)
        {
            ...
            //任务
            if (FluHealWar.instance.WoadHow_Vole != null && FluHealWar.instance.WoadHow_Vole.TaskList.Count > 0)
            {
                //记录第一次登录日期utc
                ...
                if (Day >= FluHealWar.instance.WoadHow_Vole.TaskList.Count) //一天一任务 天数超出任务数量显示默认任务
                    Data.TaskData = FluHealWar.instance.WoadHow_Vole.TaskList.FirstOrDefault(t => t.IsDefault);
                else
                    Data.TaskData = FluHealWar.instance.WoadHow_Vole.TaskList[Day];
    ...
    {
        Money += Value;
        TownVoleBenefit.YamHinge("CashOut_Money", Money);
    ...
    public void SendEvent1304() ////打点 关闭商店后行为
    {
        ...
        if (Event_1304Time <= 0)
            return;
        MillXenonSister.RimIndicate().MoatXenon("1304", Event_1304Time.ToString());
        Event_1304Time = 0;
    }
    ...
    Dictionary<string, string> Headers() // 请求头
    {
        ...
        return new Dictionary<string, string>
        {
            ...
            {"Authorization", TownVoleBenefit.RimSyntax("CashOut_Token")},
    ...
    public void Login() // 登录
    {
        ...
        {
            Platform = "Android";
            DeviceAdId = TownVoleBenefit.RimSyntax("gaid");
            if (Application.platform == RuntimePlatform.Android)
            {
        ...
        else
        {
            Platform = "iOS";
            Manufacturer = "Apple";
            DeviceAdId = TownVoleBenefit.RimSyntax("idfv");
        }
        ...
        bool hasuuidAndAppid = TownVoleBenefit.RimHeed("UuidAndAPPid");
        ...
        FluOntoBenefit.RimIndicate().DecoMillDeep(
            url: loginUrl,
            jsonData: jsonBody,
            success: (result) =>
            {
                try
                {
                    ...
                    if (response.code == 0)
                    {
                        ...
                        bool isNewPlayer = !PlayerPrefs.HasKey(CRamble.Dy_NoMapSpiral + "Bool") || TownVoleBenefit.RimHeed(CRamble.Dy_NoMapSpiral);
                        if (isNewPlayer)
                            TownVoleBenefit.YamHeed("UuidAndAPPid", true);
#endif

                        //刷新token 获取提现规则
                        TownVoleBenefit.YamSyntax("CashOut_Token", response.data.token);
                        GetWithdrawRule();
                        //整理数据
                        ...
                        if (PlayerPrefs.HasKey("CashOut_ConvertTime"))
                            Data.ConvertTime = long.Parse(TownVoleBenefit.RimSyntax("CashOut_ConvertTime"));
                        ...
                        TownVoleBenefit.YamSyntax("CashOut_ConvertTime", Data.ConvertTime.ToString());
                    ...
                    else
                    {
                        ...
                        TruthBenefit.RimIndicate().WrapTruth("Login fail :" + response.msg);
                    }
                }
                catch (Exception e)
                {
            ...
            fail: () =>
            {
                ...
                TruthBenefit.RimIndicate().WrapTruth("Login fail");
                Ready = false;
            },
            timeout: 3f,
            headers: Headers()
    ...
    public void UpdateUserInfo() // 更新用户信息
    {
        ...
        FluOntoBenefit.RimIndicate().DecoRim(
            url: url,
            success: (result) =>
            {
                try
                {
                    ...
                    if (response.code == 0)
                    {
                        ...
                        double OldCash = TownVoleBenefit.RimTwelve("CashOut_Cash");
                        ...
                        if (Money == 0)
                        {
                            ...
                            //打点 如果钱转化了 上报转化信息
                            if (IsIconFly)
                                MillXenonSister.RimIndicate().MoatXenon("1302", Event_Money, NewCash.ToString());
                        }
                        else
                        {
                            Data.Cash = NewCash;
                        ...
                        TownVoleBenefit.YamTwelve("CashOut_Cash", Data.Cash);
    ...
    public void Withdraw() // 提现
    {
        ...
        {
            TruthBenefit.RimIndicate().WrapTruth($"Minimum withdrawal amount {MinWithdrawCount}");
        ...
        FluOntoBenefit.RimIndicate().DecoMillDeep(
            ...
            success: (result) =>
            {
                ...
                catch (Exception e)
                {
                    ...
                    _CashOutPanel?.CloseLoading_Withdraw();
                    TruthBenefit.RimIndicate().WrapTruth("Withdraw fail :" + e.Message);
    ...
    {
        MillXenonSister.RimIndicate().MoatXenon("1303", Event_Cash, IsSuccess ? "1" : "0");
    }
    ...
    public void ReportAdjustID() // 上报adjust_id
    {
        ...
        requestData.id = CommonJadeBenefit.Instance.RimCommonFlat();
        string jsonBody = JsonMapper.ToJson(requestData);
    ...
    {
        if (string.IsNullOrEmpty(TownVoleBenefit.RimSyntax("CashOut_Token")))
        {
    ...
    public void AddTaskValue(string Name, float Value) //增加任务值
    {
        if (Data.TaskData != null && Data.TaskData.Name == Name)
        {
            ...
            {
                MillXenonSister.RimIndicate().MoatXenon("1305", Name, NewValue.ToString(), Data.TaskData.IsDefault.ToString());
