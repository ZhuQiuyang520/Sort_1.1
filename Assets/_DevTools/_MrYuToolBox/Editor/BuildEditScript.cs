...
public class BuildEditScript : MonoBehaviour
{
    ...
    public static void Check(int buildTargetIndex, BuildInfoData info)
    {
        ...
        if (Resources.FindObjectsOfTypeAll<FluHealWar>()[0].RoarMar != info.BaseUrl)
        {
            isSame = false;
            Debug.Log("域名");
        }
        ...
        if (Resources.FindObjectsOfTypeAll<ADBenefit>()[0].MAX_SDK_KEY != info.Applovin_SDK_KEY)
        {
            isSame = false;
            Debug.Log("admanager key");
        }
        if (Resources.FindObjectsOfTypeAll<ADBenefit>()[0].MAX_REWARD_ID != info.Applovin_REWARD_ID)
        {
            isSame = false;
            Debug.Log("reward id");
        }
        if (Resources.FindObjectsOfTypeAll<ADBenefit>()[0].MAX_INTER_ID != info.Applovin_INTER_ID)
        {
            isSame = false;
            Debug.Log("inter id");
        }
        if (Resources.FindObjectsOfTypeAll<FluHealWar>()[0].BoneLuce != info.GameCode)
        {
            isSame = false;
            Debug.Log("gamecode");
        }
    ...
    public static void SuperBuild(int buildTargetIndex, BuildInfoData info)
    {
        ...
        Resources.FindObjectsOfTypeAll<ADBenefit>()[0].MAX_SDK_KEY = info.Applovin_SDK_KEY;
        Resources.FindObjectsOfTypeAll<ADBenefit>()[0].MAX_REWARD_ID = info.Applovin_REWARD_ID;
        Resources.FindObjectsOfTypeAll<ADBenefit>()[0].MAX_INTER_ID = info.Applovin_INTER_ID;
        ...
        if (!GameObject.Find("MainManager") || !GameObject.Find("MainManager").GetComponent<CommonJadeBenefit>())
        {
            if (GameObject.Find("MainManager"))
            {
                GameObject.Find("MainManager").AddComponent<CommonJadeBenefit>();
            }
            else
            {
                GameObject mainObj = new GameObject("MainManager");
                mainObj.AddComponent<CommonJadeBenefit>();
            }
        }
        GameObject.Find("MainManager").GetComponent<CommonJadeBenefit>().TraderID = info.Adjust_APP_ID;

        if (!GameObject.Find("MainManager").GetComponent<LoadAtBenefit>())
        {
            GameObject.Find("MainManager").AddComponent<LoadAtBenefit>();
        }

#if UNITY_IOS
        GameObject.Find("MainManager").GetComponent<LoadAtBenefit>().appid = info.Rate_ID;
#endif
#if UNITY_ANDROID
        GameObject.Find("MainManager").GetComponent<LoadAtBenefit>().appid = info.PackageName;
#endif
        ...
        Resources.FindObjectsOfTypeAll<FluHealWar>()[0].BoneLuce = info.GameCode;
        Resources.FindObjectsOfTypeAll<FluHealWar>()[0].RoarMar = info.BaseUrl;        Resources.FindObjectsOfTypeAll<FluHealWar>()[0].RoarAdaptMar = info.BaseUrl + CRamble.AdaptMar;
        Resources.FindObjectsOfTypeAll<FluHealWar>()[0].RoarRambleMar = info.BaseUrl + CRamble.RambleMar;
        Resources.FindObjectsOfTypeAll<FluHealWar>()[0].RoarShowMar = info.BaseUrl + CRamble.ShowMar;
        Resources.FindObjectsOfTypeAll<FluHealWar>()[0].RoarCommonMar = info.BaseUrl + CRamble.CommonMar;
        EditorSceneManager.SaveScene(EditorSceneManager.GetActiveScene());

#if UNITY_IOS
        string url = info.BaseUrl + CRamble.RambleMar + info.GameCode + "&channel=" + "AppStore" + "&version=" + buildWindow.Version;
#elif UNITY_ANDROID
        string url = info.BaseUrl + CRamble.ConfigUrl + info.GameCode + "&channel=" + "GooglePlay" + "&version=" + buildWindow.Version;
#else
        string url = info.BaseUrl + CRamble.ConfigUrl + info.GameCode + "&channel=" + "GooglePlay" + "&version=" + buildWindow.Version;
#endif
       ...
       (data) =>
       {
           ...
           //if (rootData.data.apple_pie != "apple")
           {
               ...
               if (GameObject.Find("FluOntoBenefit"))
               {
                   DestroyImmediate(GameObject.Find("FluOntoBenefit"));
               }
           }
       },
       () =>
